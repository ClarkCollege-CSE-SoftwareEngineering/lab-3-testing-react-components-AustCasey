...
describe('TaskList', () => {
  ...
  describe('error handling', () => {
    it('shows error message when create tasks fails', async () => {
      const user = userEvent.setup();
  ...
  describe('adding a task with empty title', () => {
    it('does not call createTask API', async () => {
      const user = userEvent.setup();
      mockedTaskApi.fetchTasks.mockResolvedValue([]);
      render(<TaskList />);
      await screen.findByText(/no tasks yet/i);
      await user.click(screen.getByRole('button', { name: /add task/i }));
      expect(mockedTaskApi.createTask).not.toHaveBeenCalled();
    });
  });

  describe('deleting a non-existent task', () => {
    it('handles error gracefully', async () => {
      const user = userEvent.setup(); 
      mockedTaskApi.fetchTasks.mockResolvedValue([
        { id: '1', title: 'Existing Task', completed: false },
      ]);
      mockedTaskApi.deleteTask.mockRejectedValue(new Error('Failed to delete task'));
      render(<TaskList />);
